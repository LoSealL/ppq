...
def _convert_onnx_pads_to_torch(
    onnx_pads: List[int], mode: Optional[str] = None
) -> List[int]:
    ...
    if mode is not None:
        if mode == "1d" and len(onnx_pads) != 2:
            raise ValueError(
                "1d Operation needs 2-d padding value, "
                f"while your padding value is {onnx_pads}"
            )
        elif mode == "2d" and len(onnx_pads) != 4:
            raise ValueError(
                "2d Operation needs 4-d padding value, "
                f"while your padding value is {onnx_pads}"
            )
        elif mode == "3d" and len(onnx_pads) != 6:
            raise ValueError(
                "3d Operation needs 6-d padding value, "
                f"while your padding value is {onnx_pads}"
            )
...
def Attention_forward(
    ...
    **kwargs,
    ...
    com.microsoft.Attention
        Multi-Head Self Attention that can be either unidirectional (like GPT-2) or
        bidirectional (like BERT). The mask_index input is optional. Besides raw
        attention mask with shape(batch_size, past_sequence_length + sequence_length)
        or (batch_size, sequence_length, past_sequence_length + sequence_length) with
        value 0 for masked and 1 otherwise,

        We also support other two formats: When input has right-side padding,
        mask_index is one dimension with shape (batch_size), where value of each
        element is the end position, or valid length of actual sequence excluding
        padding.
        When input has left-side padding, mask_index has shape (2 * batch_size),
        where the values are the exclusive end positions followed by the inclusive
        start positions. When unidirectional is 1, and each token only attend to
        previous tokens. For GPT-2, both past and present state are optional.

        Present state could appear in output even when past state is not in input.

    Version
        This version of the operator has been available since version 1 of the
        'com.microsoft' operator set.
    ...
    Inputs (3 - 6)
        ...
        weight : T
            2D input tensor with shape (input_hidden_size, 3 * hidden_size), where
        ...
        mask_index (optional) : M
            Attention mask with shape (batch_size, 1, sequence_length, sequence_length),
    ...
    num_heads = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="num_heads", compulsive=True
    )
    qkv_hidden_sizes = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="qkv_hidden_sizes", default=0
    )
    unidirectional = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="unidirectional", default=0
    )
    if unidirectional != 0:
        raise NotImplementedError(
            "Attention Layer with unidirectional != 0 is not implemented."
        )
    ...
    if mask_index is not None or past is not None:
        raise NotImplementedError(
            "Attention Layer with mask_index and past != None is not implemented."
        )
...
def Conv_forward(
    ...
    **kwargs,
    ...
    r"""The convolution operator consumes an input tensor and a filter, and
    computes the output.
    ...
    Attributes
        ...
        pads : list of ints
            ...
            If not present, the padding defaults to 0 along start and end of each
            spatial axis.
    ...
    Inputs (2 - 3)
        X (differentiable) : T
            ...
            Note that this is for the 2D image. Otherwise the size is
            (N x C x D1 x D2 ... x Dn).

            Optionally, if dimension denotation is in effect,
            the operation expects input data tensor to arrive
                with the dimension denotation of [DATA_BATCH, DATA_CHANNEL,
        ...
        W (differentiable) : T
            ...
            Or in other words FILTER_IN_CHANNEL multiplied by the number of groups
            should be equal to DATA_CHANNEL and the number of feature maps M should be
            a multiple of the number of groups G.
    ...
    auto_pad = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="auto_pad", default="NOTSET"
    )
    ...
    if ndim in {2, 3}:
        if auto_pad != "NOTSET":
            raise NotImplementedError(
                f'auto_pad must be "NOTSET" with 1-d conv {op.name}'
            )
        torch_pads = _convert_onnx_pads_to_torch(onnx_pads=onnx_pads, mode="1d")
    ...
    # conv - 3d
        ...
        if auto_pad != "NOTSET":
            raise NotImplementedError(
                f'auto_pad must be "NOTSET" with 3-d conv {op.name}'
            )
        torch_pads = _convert_onnx_pads_to_torch(onnx_pads=onnx_pads, mode="3d")
        ...
        raise ValueError(
            f"Operation {op.name} is invalid, {ndim}-d input is not supported."
        )
    return output
...
def ConvTranspose_forward(
    ...
    **kwargs,
    ...
    process_attribute(
        op.attributes, values[0].shape[2:], values[1].shape[2:], "ConvTranspose"
    )
    groups = GET_ATTRIBUTE_FROM_OPERATION(op=op, attribute="group", default=1)
    onnx_pads = GET_ATTRIBUTE_FROM_OPERATION(op=op, attribute="pads", default=0)
    dilation = GET_ATTRIBUTE_FROM_OPERATION(op=op, attribute="dilations", default=1)
    stride = GET_ATTRIBUTE_FROM_OPERATION(op=op, attribute="strides", default=1)
    output_padding = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="output_padding", default=0
    )
    ...
    # 2d conv transpose
    if ndim == 4:
        torch_pads = _convert_onnx_pads_to_torch(onnx_pads=onnx_pads, mode="2d")
        if isinstance(torch_pads, list) and len(torch_pads) == 2:
...
def MaxPool2d_forward(
    ...
    **kwargs,
    ...
    """maxpool2d forward function to support indices output."""
    ASSERT_NUM_OF_INPUT(op=op, values=values, min_num_of_input=1, max_num_of_input=1)
    values = VALUE_TO_EXECUTING_DEVICE(op=op, ctx=ctx, values=values)
    ...
    ceil_mode = bool(
        GET_ATTRIBUTE_FROM_OPERATION(op=op, attribute="ceil_mode", default=False)
    )
    ...
    if op.type == "GlobalMaxPool":
        ...
        kernel_size = GET_ATTRIBUTE_FROM_OPERATION(
            op=op, attribute="kernel_shape", compulsive=True
        )
    kv: dict = dict(  # to trick pylance and mypy
    ...
    if ndim == 5:
        ...
        if isinstance(torch_pads, list) and len(torch_pads) != 3:
            x = F.pad(x, torch_pads)
            torch_pads = 0
        kv.update(padding=torch_pads)
        output = F.max_pool3d(x, **kv)

    elif ndim == 4:
        # onnx pads format[top, left, bottom, right] to torch pads
        ...
        if isinstance(onnx_pads, list) and len(onnx_pads) == 4:
            ...
            if p_left == p_right and p_top == p_bottom:
                ...
                x = F.pad(
                    x, pad=_convert_onnx_pads_to_torch(onnx_pads), value=float("-inf")
                )
                onnx_pads = 0
        kv.update(padding=onnx_pads)
        output = F.max_pool2d(x, **kv)
...
def BatchNormalization_forward(
    ...
    **kwargs,
    ...
    op_attr = {
        ...
        "momentum": 1 - op.attributes.get("momentum", 0.9),
    }
    output = F.batch_norm(
        input_data, running_mean, running_var, weight=weight, bias=bias, **op_attr
    )
    return output
...
def Mul_forward(
    ...
    **kwargs,
    ...
    Args:
        ...
        values (Sequence[torch.Tensor]): [description]
...
def MultiHeadAttention_forward(
    ...
    **kwargs,
    ...
    Args:
        ...
        values (Sequence[torch.Tensor]): opr inputs
    ...
    **kwargs,
    ...
    if op.type == "Add":
        ...
        if version < "1.5.0" or version >= "1.7.0":
            if values[0].dtype in [torch.int32, torch.int64] and values[1].dtype in [
                torch.int32,
                torch.int64,
            ]:
                if values[0].dtype == torch.int64 or values[1].dtype == torch.int64:
        ...
        raise NotImplementedError(f"Unsupported Eltwise type: {op.type}")
    return output
...
def Concat_forward(
    ...
    **kwargs,
    ...
    concat_view: Sequence[torch.Tensor] = []
    for value in values:
        if value.ndim == 0:
            value = value.unsqueeze(0)
        concat_view.append(value)

    return torch.cat(concat_view, dim=axis)
...
def Constant_forward(
    ...
    **kwargs,
    ...
    Args:
        op (Operation): [description]
        input_values (Sequence[torch.Tensor]): [description]
    ...
    **kwargs,
    ...
    if axis is not None:
        ...
        if not isinstance(repeats, list):
            repeats = [repeats]
        repeats = [int(i) for i in repeats if i is not None]
    assert input.ndim == len(repeats)
    output = input.repeat(repeats)
    return output
...
def Squeeze_forward(
    ...
    **kwargs,
    ...
    if op.opset.version >= 13:
        ASSERT_NUM_OF_INPUT(
            op=op, values=values, min_num_of_input=1, max_num_of_input=2
        )
        squeezing_tensor = values[0]
        axes = [
            axis
            for axis in range(squeezing_tensor.ndim)
            if squeezing_tensor.shape[axis] == 1
        ]
        ...
        ASSERT_NUM_OF_INPUT(
            op=op, values=values, min_num_of_input=1, max_num_of_input=1
        )
...
def Unsqueeze_forward(
    ...
    **kwargs,
    ...
    if op.opset.version >= 13:
        ASSERT_NUM_OF_INPUT(
            op=op, values=values, min_num_of_input=2, max_num_of_input=2
        )
...
def Gather_forward(
    ...
    **kwargs,
    ...
    if op.type == "Gather":
        ...
        raise NotImplementedError(f"Unsupported op type {op.type}.")
    return output
...
def ConstantOfShape_forward(
    ...
    **kwargs,
    ...
    Returns:
        torch.Tensor: [description]
    """
    value = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="value", compulsive=False, default=0.0
    )
    [shape], fill_value = values, convert_any_to_python_primary_type(value)
    assert not isinstance(fill_value, (str, list)) and fill_value is not None
...
def Slice_forward(
    ...
    **kwargs,
    ...
    Slice uses the ``starts``, ``ends``, ``axes`` and ``steps`` inputs to select a sub-
    tensor of its input data tensor.

    An effective ``starts[i]``, ``ends[i]``, and ``steps[i]`` must be computed for each
    i in ``[0, ... r-1]`` where ``r = rank(input)`` as follows:

    If ``axes`` are omitted, they are set to ``[0, ..., r-1]``. If ``steps`` are
    omitted, they are set to ``[1, ..., 1]`` of length ``len(starts)``

    The effective values are initialized as ``start[i] = 0, ends[i] = dims[i]`` where
    ``dims`` are the dimensions of ``input`` and ``steps[i] = 1``.

    All negative elements of ``axes`` are made non-negative by adding ``r`` to them,
    where ``r =rank(input)``.

    All negative values in ``starts[i]`` and ``ends[i]`` have ``dims[axes[i]]`` added
    to them, where dims are the dimensions of ``input``. Then ``start[axes[i]]`` is
    the adjusted ``starts[i]`` is clamped into the range ``[0, dims[axes[i]]]`` for
    positive stepping and ``[0, dims[axes[i]]-1]`` for negative stepping.

    The clamping for the adjusted ``ends[i]`` depends on the sign of ``steps[i]`` and
    must accommodate copying 0 through ``dims[axes[i]]`` elements, so for positive
    stepping ``ends[axes[i]]`` is clamped to ``[0, dims[axes[i]]]``, while for negative
    stepping it is clamped to ``[-1, dims[axes[i]]-1]``.
    ...
    data, starts_t, ends_t = values[:3]
    if len(values) > 3 and values[3] is not None:
        axes_t = values[3]
    else:
        axes_t = torch.arange(0, data.ndim)
    steps_t = values[4] if len(values) > 4 else torch.ones_like(starts_t)
    axes = axes_t.tolist()
    starts, ends, steps = starts_t.tolist(), ends_t.tolist(), steps_t.tolist()
    ...
    if neg_axes_slices:
        neg_axes_slices = [Ellipsis] + neg_axes_slices  # type: ignore
...
def Resize_forward(
    ...
    **kwargs,
    ...
    if len(values) == 4 and values[-1] is not None:
        size = values[-1].cpu().tolist()
    else:
        size = None
    ...
    # If 'size' is specified, then set scales to empty data (zero shape) in
    # this operator's input list.
    if size is None or len(size) == 0:
        size = None
        if len(values) == 3:
            scale_factor = values[-1].cpu()
        elif len(values) == 4 and values[2] is not None:
            scale_factor = values[2].cpu()
        else:
            raise ValueError("size and scale are all none")
...
def _NMS_forward(
    ...
    **kwargs,
    ...
    thus translating or reflections of the coordinate system result in the same boxes
    being selected by the algorithm. The selected_indices output is a set of integers
    indexing into the input collection of bounding boxes representing the selected
    boxes.
    The bounding box coordinates corresponding to the selected indices can then be
    obtained using the Gather or GatherND operation.

    Attributes
        center_point_box : int (default is 0)

        Integer indicate the format of the box data.
        The default is 0. 0 - the box data is supplied as [y1, x1, y2, x2] where
        ...
        [0, 1]) or absolute. Mostly used for TF models. 1 - the box data is supplied as
    ...
    Inputs (2 - 5)
        ...
        Float representing the threshold for deciding when to remove boxes based on
        score. It is a scalar.
    ...
    for i in range(batch):
        ...
        for j in range(num_classes):
            ...
            # In mmcv.ops.nms, boxes should given in the order of
            ...
            keep = torch.stack(
                (torch.full_like(keep, i), torch.full_like(keep, j), keep), dim=1
            )
            output.append(keep)
...
def ReduceMax_forward(
    ...
    **kwargs,
    ...
    if op.opset.version == 19:
        ASSERT_NUM_OF_INPUT(
            op=op, values=values, min_num_of_input=1, max_num_of_input=2
        )
        values = VALUE_TO_EXECUTING_DEVICE(op=op, ctx=ctx, values=values)
        input_value = values[0]
        # axis is in inputs instead of attributes in opset-version 19
        # get axis from op's inputs instead of values (may be quantized)
        dim = op.inputs[1].value.tolist()
    else:
        [input_value] = values
...
def ReduceSum_forward(
    ...
    **kwargs,
    ...
    if op.opset.version >= 13:
        ...
        if len(values) > 1:
            dim = values[1]
        keepdim, noop_with_empty_axes = bool(
            op.attributes.get("keepdims", 1)
...
def TopK_forward(
    ...
    **kwargs,
    ...
    topk = torch.topk(input=x, k=k, dim=axis, largest=largest, sorted=sorted)
...
def Expand_forward(
    ...
    **kwargs,
    ...
    output = tensor * torch.ones(
        convert_any_to_python_primary_type(shape),
        dtype=tensor.dtype,
...
def Range_forward(
    ...
    **kwargs,
    ...
    return torch.arange(start, limit, delta, device=ctx.executing_device)
...
def Softmax_forward(
    ...
    **kwargs,
    ...
    Returns:
        torch.Tensor: [description]
    """
    default_axis = -1 if op.opset.version >= 13 else 1
    ASSERT_NUM_OF_INPUT(op=op, values=values, min_num_of_input=1, max_num_of_input=1)
    [input] = values
...
def ReduceL2_forward(
    ...
    **kwargs,
    ...
    if op.opset.version == 19:
        ...
        # axis is in inputs instead of attributes in opset-version 19
        # get axis from op's inputs instead of values (may be quantized)
        axis = op.inputs[1].value.tolist()
    else:
        [input_value] = values
        axis = op.attributes.get("axes", None)
    keepdim = bool(op.attributes.get("keepdims", 1))
...
def skipLayerNormPlugin_forward(
    op: Operation,
    values: Sequence[torch.Tensor],
    ctx: Optional[TorchBackendContext] = None,
    **kwargs,
...
def Pad_forward(
    ...
    **kwargs,
    ...
    pads = pads.tolist()
    pads = _convert_onnx_pads_to_torch(pads)
...
def RoiAlign_forward(
    ...
    **kwargs,
    ...
    output = roi_align(value, boxes, output_size, spatial_scale, sampling_ratio)
    return output
...
def MMCVRoiAlign_forward(
    ...
    **kwargs,
    ...
    output_height = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="output_height", default=1
    )
    output_width = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="output_width", default=1
    )
    sampling_ratio = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="sampling_ratio", default=0
    )
    spatial_scale = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="spatial_scale", default=1.0
    )
...
def DepthToSpace_forward(
    ...
    **kwargs,
    ...
    if mode == "DCR":
        ...
        # ??? i do known why following is correct.
        output = F.pixel_shuffle(input_data, upsample)
    return output
...
def InstanceNormalization_forward(
    op: Operation,
    values: Sequence[torch.Tensor],
    ctx: Optional[TorchBackendContext] = None,
    **kwargs,
    ...
    # TODO:
    # For len(input_value)==5 not support yet, inputs = [data weight bias mean and var].
    ...
    output = F.instance_norm(
        input_data, running_mean, running_var, weight, bias, eps=eps
    )
    return output


def Parameter_forward(
    op: Operation,
    values: Sequence[torch.Tensor],
    ctx: Optional[TorchBackendContext] = None,
    **kwargs,
...
def GRU_forward(
    ...
    **kwargs,
    ...
    # sequence length will be dropped without warning.
    # if seq_len is not None: raise NotImplementedError('PPQ do not support LSTM with
    # explicit length.')

    # check attributes
    activation_alpha = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="activation_alpha", default=None
    )
    activation_beta = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="activation_beta", default=None
    )
    activations = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="activations", default=None
    )
    direction = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="direction", default="forward"
    )
    hidden_size = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="hidden_size", compulsive=True
    )
    linear_before_reset = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="layout", default=0
    )
    if linear_before_reset != 0:
        raise NotImplementedError(
            "PPQ do not support LSTM with linear_before_reset != 1."
        )
    ...
    if initial_h is None:
        initial_h = torch.zeros(
            size=[s, x.shape[1], x.shape[2]], device=x.device, dtype=torch.float32
        )
...
def LSTM_forward(
    ...
    **kwargs,
    ...
    # sequence length will be dropped without warning.
    # if seq_len is not None: raise NotImplementedError('PPQ do not support LSTM with explicit length.')
    ...
    if initial_h is None:
        initial_h = torch.zeros(
            size=[s, x.shape[1], hidden_size], device=x.device, dtype=torch.float32
        )
...
def Sum_forward(
    ...
    **kwargs,
    ...
    Returns:
        torch.Tensor: _description_
    """
    if op.precision != TargetPrecision.SOI:
