...
class TorchDbdcObserver(TorchHistObserver):
    """TorchDbdcObserver collects histogram of given tensor.
    ...
    def __init__(
        ...
        super().__init__(watch_on, quant_cfg, hist_bins)
        print(quant_cfg.detail.get("beta"))
        self.beta = quant_cfg.detail.get("beta") or 0.9
        self.clip_ratio = quant_cfg.detail.get("clip_ratio") or 1e-5
    ...
    def observe(self, value: torch.Tensor):
        ...
        if self._phase == "Detecting Minmax":
        ...
        elif self._phase == "Collating Dbdc":
            ...
            self.density_based_dual_clipping(self._hist)

    def density_based_dual_clipping(self, histogram):
        hist_sum = torch.sum(histogram)
        assert self.l is not None and self.u is not None
        l, u = self.l, self.u
        delta = max(1, int((u - l) / self._hist_bins))
        while torch.sum(histogram[l:u]) / hist_sum >= (1 - self.clip_ratio):
            if histogram[l] < histogram[u - 1]:
                l += delta
            else:
                u -= delta
        self.l = int(self.beta * self.l + (1 - self.beta) * l)
        self.u = int(self.beta * self.u + (1 - self.beta) * u)
    ...
    def hist_to_scale_offset(
        ...
        hist_sum = torch.sum(histogram)
        for bin_range in range(quant_bins, hist_bins, quant_bins):
            p_hist = torch.zeros(
    ...
    def render_quantization_config(self):
        ...
        if self._phase == "Detecting Minmax":
            ...
            if self._quant_cfg.policy.has_property(QuantizationProperty.SYMMETRIC):
                ...
                self.l, self.u = 0, self._hist_bins
            else:
                hist_range = max_val - min_val
                self.l, self.u = min_val, max_val  # values
            self._min = min_val
            self._max = max_val
            self._hist_scale = hist_range / self._hist_bins
            self._phase = "Collating Dbdc"
        elif self._phase == "Collating Dbdc":
            # Dbdc Hist
            assert self._hist is not None
            # fmt: off
            self._hist = self._hist[self.l:self.u]
            # fmt: on
            print(
                f"Through DBDC algo, histogram min_bin is:{self.l}, max_bin is:{self.u}"
            )
            scale, offset = self.hist_to_scale_offset(
                histogram=self._hist,
                hist_bins=len(self._hist),
