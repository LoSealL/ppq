...
class PassiveParameterQuantizePass(QuantizationOptimizationPass):
    ...
    other tensors. This pass process 4 types of passive parameter by default, namely:
        ...
        Padding Value, must has a scale = input scale
    ...
    * pad_visiblity(bool)
    ...
    def __init__(
        ...
        clip_visiblity: QuantVisibility = QuantVisibility.INTERNAL,
        pad_visiblity: QuantVisibility = QuantVisibility.INTERNAL,
    ):
        ...
        for op in graph.operations.values():
            ...
            if op.type in {"Conv", "ConvTranspose", "Gemm"} and self.process_bias:
                ...
                if op.num_of_input == 3:
                    ...
                    if bias is None:
                        raise ValueError(
                            f"Bias Variable {op.inputs[-1].name} must be a constant. "
                    ...
                    if (
                        op.inputs[-1].value.ndim == 0
                        and op.inputs[-1].value.numel() == 1
                    ):
