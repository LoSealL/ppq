...
from mppq.quantization.algorithm.experimental import BanditDelegator
...
class LearningToCalibPass(TrainingBasedPass):
    ...
    def calib_block(
        ...
        dataset = RandomMemDataset(
            data=[[qt, fp] for qt, fp in zip(quant_inputs, fp32_outputs)]
        )
        ...
        for delegator in delegators:
            executor.register_quantize_delegate(
                config=delegator.config, delegator=delegator
            )
        ...
        with tqdm(total=self.target_step) as t:
            while cur_iter < self.target_step:
                ...
                qt_output = executor.partial_graph_forward(
                    ...
                    feed_dict={input_var.name: qt_input},
                    output_names=[output_var.name],
                )[0]
                ...
                if cur_iter % 50 == 0:
                    t.set_description(
                        desc=f"Block [{self._bidx + 1}/{self._num_of_blocks}]"
                    )
        ...
        **kwargs,
        ...
        for op in graph.operations.values():
            ...
            if not any([rp.is_computing_op for rp in block.rps]):
                continue
            if not block.sp.is_computing_op:
                continue
...
class MatrixFactorizationPass(QuantizationOptimizationPass):
    ...
    def train_for_factorization(
        ...
        w: torch.Tensor,
        penalty=0.1,
        executing_device: str = "cuda",
        max_iter: int = 100000,
    ):
        assert w.ndim == 2

        a, b = torch.rand(size=[w.shape[0], w.shape[1]]), torch.rand(
            size=[w.shape[1], w.shape[1]]
        )
        ...
        for _ in tqdm(range(max_iter), "Training for factorization ..."):
            penalty_loss = (
                torch.mean(torch.square(a)) + torch.mean(torch.square(b))
            ) * penalty
        ...
        **kwargs,
        ...
        for name in self.interested_layers:
            if name not in graph.operations:
                raise ValueError(
                    f"Can not Split layer {name}, can not find it in current graph."
                )

        for operation in graph.operations.values():
            if operation.name in self.interested_layers:
                assert operation.type in {
                    "Conv",
                    "Gemm",
                }, "Can not split layer, cause layer type is not support"
        ...
        for operation in splitting_layers:
            ...
            graph.insert_op_on_var(inserting_op=split, var=operation.outputs[0].name)
            if operation.outputs[0].name in graph.outputs:
                graph.outputs.pop(operation.outputs[0].name)
                graph.outputs[split.outputs[0].name] = split.outputs[0]

            # add weight link
            spilted_w = Variable(
                name=split.name + "_weight", value=b, is_parameter=True
            )
            graph.append_variable(spilted_w)

            split.inputs.append(spilted_w)
            spilted_w.dest_ops.append(split)

            # if has bias, relink bias
            if len(operation.parameters) > 1:
                bias_var = operation.parameters[-1]
                bias_var.dest_ops.remove(operation)
                bias_var.dest_ops.append(split)
                split.inputs.append(bias_var)
