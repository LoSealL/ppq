...
from mppq.quantization.observer import OBSERVER_TABLE, OperationObserver
    ...
    * override(bool)
    ...
    def calibrate(
        ...
        with tqdm(total=self._calib_steps, desc=desc) as progressing_bar:
            for _ in range(ceil(self._calib_steps / len(dataloader))):
                for data in dataloader:
                    if self._collate_fn is not None:
                        data = self._collate_fn(data)
                    executor.forward(
                        inputs=data, hooks=hooks, output_names=output_names
                    )
    ...
    def optimize(
        ...
        **kwargs,
        ...
        for op_name, operation in graph.operations.items():
            ...
            assert graph is executor._graph
    ...
    def optimize(self, graph: BaseGraph, **kwargs) -> None:
        if self.variables is None:
            for op in graph.operations.values():
                if op.type == "Softmax" and isinstance(op, QuantableOperation):
                    ...
                    if (
                        ...
                        if self.verbose:
                            print(
                                f"Calibration Method of Op {op.name} "
            ...
            if not isinstance(self.variables, list):
                raise TypeError(
                    "Isotone Calibration Pass needs a list of variable name as its input."
                )
            for var in self.variables:
                ...
                if isinstance(var, QuantableVariable):
                    ...
                    var.source_op_config.detail[OBSERVER_ISOTONE_OBSERVER_AXIS] = (
                        self.axis
                    )
                    if self.verbose:
                        print(
