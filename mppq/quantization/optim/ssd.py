...
from mppq.quantization.observer import (
    OBSERVER_TABLE,
    CalibrationHook,
    OperationObserver,
)
from mppq.quantization.qfunction import BaseQuantFunction, PPQuantFunction
...
class SSDEqualizationPass(QuantizationOptimizationPass):
    ...
    def __init__(
        ...
        quant_func: BaseQuantFunction = PPQuantFunction,
        iteration: int = 3,
    ):
        ...
        Args:
            ...
            quant_func (BaseQuantFunction, optional): quantization function. Defaults to PPQuantFunction.
        ...
        **kwargs,
        ...
        for i in range(self.iteration):
            ...
            for _, pair in tqdm(
                ...
                logger.debug(
                    f"Now Processing Pair {_ + 1}/{len(all_pairs)}: {'--'.join([op.name for op in pair])}"
                )
                self.store_parameter(pair)

                logger.debug("Collecting Activation Range for Pair...")
