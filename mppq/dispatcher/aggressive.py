...
class AggressiveDispatcher(GraphDispatcher):
    ...
    ATTENTION:
        ...
        platform attribute will greatly affect quantizer's quantization logic,
    ...
    def dispatch(
        ...
        **kwargs,
        ...
        quant_operations = search_engine.opset_matching(
            ...
            ep_expr=lambda op: op.type in {"Shape", "TopK", "NonMaxSuppression"}
            or op.is_boundary,
            direction="down",
        )
        # remove shape operations from computing ops.
        quant_operations.filter(lambda op: op.type == "Shape")
        ...
        shape_forward_matching = search_engine.opset_matching(
            ...
            rp_expr=value_tracing_pattern,
            ep_expr=lambda op: (
                op in receivers or op in quant_operations or op.is_boundary
            ),
            direction="down",
        )
        ...
        while True:
            ...
            shape_backward_matching = search_engine.opset_matching(
                sp_expr=lambda op: (
                    ...
                    and op.type != "Shape"
                    and op not in quant_operations
                ),
                rp_expr=reverse_tracing_pattern,
                ep_expr=lambda op: (
                    op in generators or op in quant_operations or op.is_boundary
                ),
                direction="up",
            )
