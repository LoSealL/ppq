    ...
    * ---------------------- *
        ...
        for op in self.graph.operations.values():
            ...
            for link in socket.links:
                ...
                for dest_op in dest_ops:
                    fanout.add(dest_op)
            (
                self._precomputed_op_fanin[op.name],
                self._precomputed_op_fanout[op.name],
            ) = (fanin, fanout)
            self.sockets[op.name] = socket

    def solve_transitive_closure(
        self, sources: Set[Operation], recursive: bool = True
    ) -> Set[Operation]:
        r"""从指定节点出发解传递闭包，该方法是量化子图切分的核心函数
    ...
    def parse_transitive_fanout(self, parsing_from: Set[Operation]) -> Set[Operation]:
        ...
        fanout = self._search_engine.opset_matching(
            ...
            rp_expr=lambda x, y: x in self._precomputed_op_fanin[y.name],
            ep_expr=None,
            direction="down",
        )
        return fanout

    def parse_transitive_fanin(self, parsing_from: Set[Operation]) -> Set[Operation]:
        r"""从指定节点出发，寻找所有扇出节点
        ...
        fanin = self._search_engine.opset_matching(
            sp_expr=lambda op: op in parsing_from,
            rp_expr=lambda x, y: y in self._precomputed_op_fanin[x.name],
            ep_expr=None,
            direction="up",
        )
        return fanin
