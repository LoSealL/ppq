...
class PointPattern(Protocol):
    """A pattern to match a single point operation."""

    def __call__(self, op: Operation) -> bool:
        return False


class RelayPattern(Protocol):
    """A pattern to match two relay operations."""

    def __call__(self, x: Operation, y: Operation) -> bool:
        return False
...
class TraversalCommand(GraphCommand):
    r"""TraversalCommand 是一个用于表示图检索指令的结构体
        一个最简单的例子：
...
class GraphPattern:
    r"""Pattern Tree 是一个用来表示图模式的结构体 这将在图中检索任意一个子图.
    ...
    def __init__(
        self, node_patterns: List[PointPattern], edges: List[List[int]]
    ) -> None:
        for idx, node_pattern in enumerate(node_patterns):
            if isinstance(node_pattern, str):
                node_patterns[idx] = TypeExpr(node_pattern)
            elif not callable(node_pattern):
                raise TypeError(
                    "Can not create Pattern with node pattern "
                    f"{str(node_pattern)} it is not callable."
                )
        ...
        for edge in edges:
            if not isinstance(edge, tuple) and not isinstance(edge, list):
                raise TypeError(
                    ...
                    "it is not tuple or list."
                )
            if len(edge) != 2:
                raise ValueError(
                    f"Can not create Pattern with edge {str(edge)} it should contains "
                    f"exact 2 elements, however {len(edge)} was given."
                )
            sp, ep = edge
    ...
    def compile(self, node_patterns: List[PointPattern], edges: List[List[int]]):
        """Pattern Compile.
        ...
        for edge in edges:
            ...
            if sp >= len(node_patterns) or sp < 0:
                raise IndexError(
                    ...
                    f"Except Value between 0 and {len(node_patterns) - 1}, "
                    f"however {sp} was given."
                )
            if ep >= len(node_patterns) or ep < 0:
                raise IndexError(
                    f"Can not Compile Pattern, Edge {edge} Out of Node Range, "
                    f"Except Value between 0 and {len(node_patterns) - 1}, however "
                    f"{ep} was given."
                )
        ...
        for _ in range(len(visited)):
            ...
            for i in forward_table[current]:
                num_of_inputs[i] -= 1
        ...
        if all(visited):
            if len(roots) > 1:
                warning(
                    ...
                    "Complex Pattern might cause memory overflow ..."
                )
...
class SearchableGraph(GraphCommandProcessor):
    ...
    def process(self, command: GraphCommand) -> Any:
        if isinstance(command, TraversalCommand):
            ...
            raise TypeError(
                "To execute a traversal-based pattern matching, a TraversalCommand "
                "is required here.\n"
    ...
    def _opset_matching(
        ...
        for op in following_ops:
            ...
            if ep_expr is not None and ep_expr(op):
                ...
                # if operation is not a valid relay point, end searching here.
                if not rp_expr(start_point, op):
                    # new feature with ppq 0.6.5, if ep_expr is None,
                    # means search until mismatch.
                    if ep_expr is None:
                        ret_collection.add(start_point)
                    continue
    ...
    def opset_matching(
        ...
        r"""opset_matching is used for operator set searching, it returns
    ...
    def concat_matching(
        self, relay_pattern: RelayPattern, end_pattern: PointPattern
    ) -> Dict[str, List[str]]:
        matchings = self.path_matching(
    ...
    def pattern_matching(
        self,
        patterns: Sequence[PointPattern | str],
        edges: List[List[int]],
        exclusive: bool = True,
    ) -> List[List[Operation]]:
        ...
        """
        pattern = GraphPattern(
            node_patterns=[TypeExpr(i) if isinstance(i, str) else i for i in patterns],
            edges=edges,
        )

        return PatternMatchHelper().match_burte_force(
            graph=self.graph, pattern=pattern, exclusive=exclusive
        )
